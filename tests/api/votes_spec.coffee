...
.on post: "/api/vote", body: {type: "up", review: 1 }
...
.on post: "/api/vote", body: {type: "down", review: 1 }
.withCSRF().withAJAX().withUser 'snow'
.is (rq) ->
  rq.expectStatus 200
    .expectJSON {score: -1, cancelled: false}
...
.on post: "/api/vote", body: { type: "up", comment: 5 }
.withCSRF().withAJAX().withUser 'snow'
.is (rq) ->
  rq.expectStatus 200
    .expectJSON {score: 1, cancelled: false}
...
.on post: "/api/vote", body: { type: "down", comment: 5 }
...
.on post: "/api/vote", body: { type: "up", review: 1000 }
.withCSRF().withAJAX().withUser 'snow'
.is (rq) -> rq.expectStatus 400

test "Vote on review with wrong direction"
.on post: "/api/vote", body: { type: "foo", review: 1000 }
.withCSRF().withAJAX().withUser 'snow'
.is (rq) -> rq.expectStatus 400

test "Vote on inexistant comment"
.on post: "/api/vote", body: { type: "up", comment: 1000 }
.withCSRF().withAJAX().withUser 'snow'
.is (rq) -> rq.expectStatus 400

test "Vote on comment with wrong direction"
.on post: "/api/vote", body: { type: "bar", comment: 5 }
.withCSRF().withAJAX().withUser 'snow'
.is (rq) -> rq.expectStatus 400

test "Vote without type"
.on post: "/api/vote", body: { comment: 5 }
.withCSRF().withAJAX().withUser 'snow'
.is (rq) -> rq.expectStatus 400

test "Vote without comment"
.on post: "/api/vote", body: { type: "up" }
.withCSRF().withAJAX().withUser 'snow'
.is (rq) -> rq.expectStatus 400
