...
.on post: "/api/review", followAllRedirects: true, data: { course_id: 737, title: "Lorem ipsum", comment: "dolor sit" }
.withCSRF().withUser "snow"
.is (rq) ->
  rq.addHeader "referer", "/en/course/neutronics-737"
    .expectBodyContains "Please grade at least"

test "Cannot post without course_id"
.on post: "/api/review", followAllRedirects: true, data: { title: "Lorem ipsum", comment: "dolor sit" }
.withCSRF().withUser "snow"
.is (rq) ->
  rq.addHeader "referer", "/api/errors"
    .expectBodyContains "course id field is required"

test "Cannot post with a comment but without a title"
.on post: "/api/review", followAllRedirects: true, data: { course_id: 737, comment: "dolor sit" }
.withCSRF().withUser "snow"
.is (rq) ->
  rq.addHeader "referer", "/api/errors"
...
.on post: "/api/review", followAllRedirects: true, data: { course_id: 921, lectures_grade: 3 }
.withCSRF().withUser "snow"
.is (rq) ->
  rq.expectBodyContains "already reviewed this course."

test "Cannot give out of bounds grades"
.on post: "/api/review", followAllRedirects: true, data: { course_id: 737, lectures_grade: -1, exercises_grade: 6 }
.withCSRF().withUser "snow"
.is (rq) ->
  rq.addHeader "referer", "/api/errors"
...
.on post: "/api/review", followAllRedirects: true, data: { course_id: 737, lectures_grade: 2 }
.withUser "snow"
